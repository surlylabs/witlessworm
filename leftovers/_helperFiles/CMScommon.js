...
function CHARCODE_isLowerCase(charCode) {
	return ((charCode >= 97) && (charCode <= 122));
}

function CHARCODE_toUpperCase(charCode) {
	if (CHARCODE_isLowerCase(charCode)) {
		return charCode - 32;
	}
	return charCode;
}

function CHARCODE_isSpecial(charCode) {
...
function CHAR_isLowerCase(ch) {
	return STRING_containsChar(STRING_ALPHA_LOWER_CASE,ch);
}

// String methods

var STRING_numeric          = '0123456789';
var STRING_alpha_upper_case = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
var STRING_alpha_lower_case = 'abcdefghijklmnopqrstuvwxyz';
var STRING_alpha            = STRING_alpha_upper_case + STRING_alpha_lower_case;
var STRING_alphanumeric     = STRING_alpha + STRING_numeric;
...
function STRING_containsChar(str,ch) {
...
function EVENT_getKeyPressCharCode(event) {
	if (event) {
		if (event.keyCode) { // IE 5
...
function EVENT_setKeyPressCharCode(event,charCode) {
	if (event) {
		if (event.keyCode) { // IE 5
			event.keyCode = charCode;
		} else if (event.which) { // Netscape
			event.which = charCode; // Doesn't work
		}
...
function TEXT_handleKeyPress(text,event,validChars,forceUpperCase) {
	var charCode = EVENT_getKeyPressCharCode(event);
	if (charCode) {
		if (!CHARCODE_isSpecial(charCode)) {
			if (forceUpperCase) {
				charCode = CHARCODE_toUpperCase(charCode);
				EVENT_setKeyPressCharCode(event,charCode);
			}
			var ch = String.fromCharCode(charCode);
			if (validChars) {
				if (!STRING_containsChar(validChars,ch)) {
					return false;
				}
			}
		}
...
function TEXT_handleChange(text) {
...
function SELECT_navigate(select) {
